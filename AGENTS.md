...
- On Windows, short-lived CLI subcommands that already finished their real work can still crash on exit if you force `process.exit(...)` while libuv async handles are closing. For command-style flows like `kanban task create`, prefer setting `process.exitCode` and letting Node drain naturally unless you specifically need an immediate hard exit.
